<template>
        ...
        <div class="card-body" >
          <div class="col-md-12 rowData">
            <div class="col-md-6">
              ...
              <button style="margin-bottom: 20px" class="btn btn-primary" type="button" 
              @click="addEmployee()"
              >
                Create Product
              </button>
            ...
            <div class="mb-3 col-md-6 rowData justify-content-end">
              <label for="table-complete-search" class="col-xs-3 col-sm-auto col-form-label colFormLabel ">Search:</label>
              <div class="col-xs-3 col-sm-auto">
                <!-- <input id="table-complete-search" type="text" @keyup="filter()" class="form-control" -->
                <input id="table-complete-search" type="text" class="form-control"
                  v-model="filterQuery" @keyup="filter()" />
              </div>
            </div>
          </div>
          <div class="table-responsive theme-scrollbar data-table">
  
            <table class="table table-striped table-bordered" v-if="mapData != null">
              <thead>
                <tr>
                  <!-- <th>S/N</th> -->
                  <!-- <th>name</th>
                      <th>Mobile no</th>
                       <th>date</th>
                        <th>city</th> -->
                  <!-- {{keyValue}} -->
                  <th scope="col" v-for="tt in keyValue">{{ tt }}</th>
                ...
                <tr v-for="(data, index) in mapData" :key="data.id">
                  ...
                  <td style="text-transform: capitalize;" v-for="(ir, i, u) in data" :key="u" v-if="u !== '0'">
                    <div v-if="i != 'pro_productImage'">
                        ...
                        <div v-if="i == 'pro_productImage'">
                          ...
                          <div v-if="ir != null">
                            <img style="height:50px; width:50px" :src="(ir)" >
                          </div>
                          <div v-if="ir == null">
                            <img style="height:50px; width:50px" :src="('https://c8.alamy.com/comp/D34CTX/confused-and-sad-question-mark-D34CTX.jpg')" >
                          </div>
                  ...
                  <td>
                    <div class="icons d-flex">
                      <div>
                        <i className="fa fa-pencil-square-o fa-lg" @click="EditForm(data.id)"></i>
                      </div>
                      <!-- <div>
                        <i className="fa fa-eye fa-lg" @click="ViewForm(data.id)"></i>
                      </div> -->
                      <div>
                        <i className="fa fa-trash-o fa-lg" @click="DeleteForm(data.id)"></i>
                      </div>
                    </div>
                  </td>
                </tr>
          ...
          <div v-if="mapData != null">
            <ul class="pagination justify-content-end pagination-primary">
               <li style="margin-top: 2px; margin-right: 5px;" class="page-item d-flex">
                <span class="mt-4  mx-1" style="color:black;">Total Count 
                   <button style="border: none; margin-top: 2px;
           border-radius: 7px; color: white; background: #0d6efd;">{{ pagination.total }}</button></span>
  ...
  export default {
    ...
    methods: {
      ...
      removePrefix(data) {
        if (data) {
          return data.split('_')[1] ? data.split('_')[1] : data.split('_')[0];
        }
        return true;
      },
      isMatchingValue(u) {
      ...
      filter() {
        ...
        if (this.filterQuery.length>0) {
          ...
          axios
            .get(
              `${config.apiUrl}/api/product-form`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
